...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-dmc-dark font-sans text-dmc-cream overflow-x-hidden">
      ...
      <section className="relative h-screen flex flex-col items-center justify-center px-6 text-center">
        <div className="animate-float-slow transition-transform">
           <h2 className="text-dmc-gold tracking-[0.3em] text-sm md:text-lg mb-4 uppercase font-light">
             DMC Empire Luxury
           </h2>
           <h1 className="font-serif text-5xl md:text-8xl mb-6 leading-tight">
             Beyond <br /> 
             <span className="text-dmc-gold italic">Expectations</span>
           </h1>
           <p className="max-w-xl mx-auto text-dmc-cream/70 text-lg mb-10 leading-relaxed font-light">
             Découvrez l'excellence capillaire et l'art de l'onglerie haut de gamme à Douala.
           </p>
           <button className="bg-gradient-to-r from-dmc-gold to-dmc-goldLight text-dmc-dark px-10 py-4 rounded-full font-bold uppercase tracking-widest hover:scale-105 transition-all shadow-2xl shadow-dmc-gold/20">
      ...
      {/* Promotion Bar */}
      <div className="bg-dmc-gold/10 border-y border-dmc-gold/30 py-4 overflow-hidden whitespace-nowrap">
        <div className="animate-[marquee_20s_linear_infinite] inline-block px-4">
          <span className="mx-8 font-serif text-dmc-gold">OFFRE EXCLUSIVE : UN BANDEAU ET UN KIT PRESS-ON NAILS OFFERTS AUX 10 PREMIÈRES CLIENTES !</span>
          <span className="mx-8 font-serif text-dmc-gold">OFFRE EXCLUSIVE : UN BANDEAU ET UN KIT PRESS-ON NAILS OFFERTS AUX 10 PREMIÈRES CLIENTES !</span>
        </div>
      </div>
      ...
      <section className="py-24 px-6 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row items-end justify-between mb-16 gap-4">
          <div className="max-w-md">
            ...
            <div className="w-20 h-1 bg-dmc-gold"></div>
          </div>
          <p className="text-dmc-cream/50 italic">Faites glisser pour explorer</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {categories.map((cat) => (
            <div 
              key={cat.id} 
              className="group relative h-[500px] rounded-3xl overflow-hidden border border-dmc-gold/20 animate-float-slow"
            ...
            >
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-dmc-dark via-dmc-dark/20 to-transparent"></div>
              
              {/* Content */}
              <div className="absolute bottom-8 left-8 right-8">
                <span className="text-dmc-gold text-xs uppercase tracking-widest mb-2 block">{cat.subtitle}</span>
                <h3 className="font-serif text-2xl mb-4">{cat.title}</h3>
                <button className="bg-dmc-cream/10 backdrop-blur-md border border-dmc-cream/20 text-dmc-cream px-6 py-2 rounded-xl text-sm group-hover:bg-dmc-gold group-hover:text-dmc-dark transition-all">
                  Voir la collection
                </button>
              </div>
            </div>
      ...
      <section className="py-24 bg-[#2b1b17]/50 text-center px-6">
        ...
        <a 
          ...
          className="inline-flex items-center gap-4 text-dmc-gold border border-dmc-gold/50 px-8 py-4 rounded-2xl hover:bg-dmc-gold hover:text-dmc-dark transition-all"
        >
      ...
      <footer className="py-10 text-center text-dmc-cream/30 text-xs tracking-widest uppercase">
...
// 1.  **Immersion Totale :** Le fond "Gold Dust" qui pulse doucement crée une ambiance de luxe immédiate.
